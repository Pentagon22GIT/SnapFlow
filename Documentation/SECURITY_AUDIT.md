...
v1.2.0は利用者情報を開発者へ送信しません。画面画像は機密情報・個人情報を含み得るため、初期OFF、ローカルメモリ限定、外部送信なしを維持します。境界ハンドルと連動リサイズ用ガイドは画面画像を取得しません。
...
- 確定とキャンセルの補正中は、全対象を画面キャプチャを使わないブラーとアイコンで一時的に覆い、途中フレームの変化を見えにくくします。
- Escape、Space変更、設定無効化、停止、別操作による無効化では世代番号を更新し、古い非同期完了処理を受理しません。
- ネットワーク通信、テレメトリー、永続ログ、Helper、XPC、Daemon、追加権限は導入していません。

本追補はソースの静的監査です。v1.2.0を公開する前に、macOS上でSwiftテスト、署名付きビルド、重いWeb動画ウィンドウ、複数Space、複数ディスプレイ、最小サイズ制約を持つアプリを含む実機試験が必要です。
