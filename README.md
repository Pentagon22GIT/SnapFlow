...
スナップ済みウィンドウの共有辺をリサイズすると、三分割と四分割を構成する縦線または横線を単位として連動します。各段の境界がすでに揃っていれば同じ線上のウィンドウが一緒に追従し、境界がずれている場合は操作中の辺が別の段の境界へ到達した時点で同じ線へ合流します。全高または全幅を占めるウィンドウがある三分割では、途中の段だけを変更できないため、関係する段を一組として扱います。

操作中は利用者が掴んでいるウィンドウを優先します。連動先はぼかし、青いガイド、アプリアイコンによる仮想ボックスで示し、MouseUp後に操作ウィンドウの最終フレームが安定してから実際のサイズを変更します。仮想ボックスには画面収録を使用しません。三分割以上でも一つの共有パネル内でブラー、ガイド、アイコンの順に描画し、別の対象のブラーがガイドを覆わないようにしています。ドラッグ中の枠はイージングで遅延させず、最新位置へ直接追従します。

共有辺でMouseDownした時点に限って追従関係と非表示の仮想ボックスを準備します。追従中は操作対象だけのWindowServerフレームを最大60Hzで取得し、複数の仮想ボックスを一回の描画更新で反映します。同じフレームは再描画せず、Accessibilityの連続照会は行いません。MouseUp後はAccessibilityで共有境界を厳密に再確認し、必要な場合は同じ確定フレームを一度再要求します。配置処理中の一時的なフレームは離脱判定へ使用せず、全対象の実フレームが確定してから記録を同時に更新します。完成状態のスナップ判定許容値は変更しません。
...
- スナップ済みウィンドウの連動リサイズ（初期ON）
- 左右端で待機した際の四分割判定拡張
- 拡張までの待機時間：0.5〜5.0秒
- 画面端の反応幅：8〜80pt
- 四隅の反応範囲：60〜300pt
- レイアウトの重なり許容度：10〜90%（初期50%）
- 各操作のグローバルショートカット
