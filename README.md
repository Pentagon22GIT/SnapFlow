...
スナップ済みウィンドウが共有する縦線または横線につまみを表示します。共有している辺全体が操作範囲になり、三分割では実際に接する区間だけ、四分割で高さや幅が一致する線は端から端までつながります。交差点では最初に動かした方向から操作する軸を決めます。つまみと青いガイドはマウス座標へ直接追従するため、重いアプリの描画が遅れてもガイドの位置計算には影響しません。
...
初期設定は「標準」です。実リサイズでは処理中の古い目標を蓄積せず、常に最新の目標だけを送ります。アイコン表示の対象にはドラッグ中のAccessibilityリサイズ要求を送りません。確定とキャンセルでも表示モードを維持し、新しくブラー対象を増やしません。位置とサイズの確認に失敗した場合は操作前のフレームへ戻します。

つまみの仮想表示には画面収録を使用しません。通常のウィンドウ境界を直接動かした場合はmacOS標準の単独リサイズとして扱い、そのウィンドウを現在の連動関係から外します。設定の「ドラッグ中なら分割へ戻せる」を有効にすると、共有境界を行き過ぎても同じドラッグ中に許容範囲へ戻した場合だけ連動を継続できます。初期設定は無効です。
