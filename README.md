...
スナップ済みウィンドウが共有する縦線または横線につまみを表示します。共有している辺全体が操作範囲になり、三分割では実際に接する区間だけ、四分割で高さや幅が実質的に一致する線は端から端までつながります。高さが異なる左右の横線は独立したまま操作できます。交差点では最初に動かした方向から操作する軸を決めます。つまみと青いガイドはマウス座標へ直接追従するため、重いアプリの描画が遅れてもガイドの位置計算には影響しません。

別のウィンドウが共有辺を覆っている場合、覆われた区間にはつまみも透明な入力領域も置きません。見えている区間だけを操作でき、設定画面を開いている間はつまみを一時的に隠します。参加ウィンドウはID、PID、現在フレームを一対一で照合し、一時的な記録フレームとの差だけではつまみを破棄しません。
...
初期ONの「スナップ中のウィンドウクリックによる最前面移動」を有効にすると、スナップ中のウィンドウを通常クリックした場合も、現在接続されているグループだけを前面へまとめます。すでにグループ全体が前面にある場合は再前面化せず、OFFの場合はクリックしたウィンドウ以外を動かしません。一時的な重なりやZ順変化だけで共有接続を解除することはありません。
