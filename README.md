...
スナップ済みウィンドウが共有する縦線または横線につまみを表示します。共有している辺全体が操作範囲になり、三分割では実際に接する区間だけ、四分割で高さや幅が実質的に一致する線は端から端までつながります。高さが異なる左右の横線は独立したまま操作できます。交差点では最初に動かした方向から操作する軸を決めます。つまみと青いガイドはマウス座標へ直接追従するため、重いアプリの描画が遅れてもガイドの位置計算には影響しません。つまみは待機時45%、接近時92%、ドラッグ中100%で表示します。初回の遮蔽確認中は18%かつ入力無効です。

別のウィンドウが共有辺の一部でも覆っている場合、その共有辺のつまみと入力領域を全体ごと一時的に隠します。別ウィンドウの脇へつまみを逃がさないため、間にあるウィンドウを越えて参加ウィンドウだけを前面化する操作は開始されません。MouseDown直前にもWindow Serverの状態を再検証します。設定画面を開いている間もつまみを一時的に隠します。参加ウィンドウはID、PID、現在フレームを一対一で照合し、一時的な記録フレームとの差だけでは接続を破棄しません。
...
表示モードと復帰設定は「分割ウィンドウを一緒にリサイズ」の子設定です。親設定がOFFの間は保存値を変更せず無効表示し、再度ONにすると以前の選択を復元します。

つまみの仮想表示には画面収録を使用しません。通常のウィンドウ境界を直接動かした場合はmacOS標準の単独リサイズとして扱い、そのウィンドウを現在の連動関係から外します。設定の「スナップがタブの最小サイズを侵害した時にスナップを保持する」は、連動リサイズがONの状態で、スナップ済みウィンドウの共有境界をウィンドウ側から直接動かした場合だけ作用します。有効にすると、共有境界を行き過ぎても同じドラッグ中に許容範囲へ戻した場合だけ連動を継続できます。初期設定は無効です。

スナップが成功すると、現在の共有境界でつながったウィンドウだけを前面へまとめ、今回操作したウィンドウを最後に最前面へ戻します。無関係なウィンドウの順序は変更しません。

初期ONの「スナップ中のウィンドウクリックによる最前面移動」を有効にすると、スナップ中のウィンドウを通常クリックした場合も、現在接続されているグループだけを前面へまとめます。macOS側の通常クリックがZ順へ反映された後に判定し、必要な場合は前面化結果を一度だけ再確認します。保留要求は最新一件だけを残します。すでにグループ全体が前面にある場合は再前面化せず、OFFの場合はクリックしたウィンドウ以外を動かしません。一時的な重なりやZ順変化だけで共有接続を解除することはありません。
...
「スナップ保持の許容率」は復帰設定の子ではなく、新しいスナップ、共有つまみ、ウィンドウ境界の直接操作に共通する最小サイズ判定です。このため連動リサイズがOFFでも通常のスナップ判定には使用されます。
...
```zsh
shasum -a 256 -c SnapFlow-1.2.1.sha256
```
...
v1.2.1では、表示されたBundle IDが`dev.pent.SnapFlow`であること、証明書SHA-1が`26479A3C344B9500A9CEFDBD00FB1A086C3D1295`であること、Designated Requirementと`release-manifest.json`が同じ値を示すことを確認します。SHA-1はこの証明書をDesignated Requirement内で識別するために使用しており、配布ファイルの完全性確認にはSHA-256を使用します。名前だけの一致は公式版の証明になりません。
...
- 分割ウィンドウの連動リサイズ（初期ON）
- スナップ中のウィンドウクリックによる最前面移動（初期ON）
- ドラッグ中の表示：軽量／標準／すべて表示（初期は標準）
- スナップがタブの最小サイズを侵害した時にスナップを保持する：連動リサイズON時だけ有効、初期OFF
- 左右端で待機した際の四分割判定拡張
- 拡張までの待機時間：0.5〜5.0秒
- 画面端の反応範囲：8〜80pt
- 四隅の反応範囲：60〜300pt
- スナップ保持の許容率：10〜90%（初期50%）
- 各操作のグローバルショートカット
